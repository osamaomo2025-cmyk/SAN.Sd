      ...
      {(role === UserRole.GOVERNMENT_MINISTER) && (
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            <div className="bg-white p-6 rounded-3xl border border-gray-200 shadow-sm flex flex-col justify-between min-h-[140px] hover:scale-[1.01] transition-all duration-300">
              ...
              <p className="text-3.5xl font-black text-[#1A1A1A] my-2">{companies.length + factories.length}</p>
            ...
            <div className="bg-white p-6 rounded-3xl border border-gray-200 shadow-sm flex flex-col justify-between min-h-[140px] hover:scale-[1.01] transition-all duration-300">
              ...
              <p className="text-3xl font-black text-sudan-green my-2">{(companies.reduce((sum, c) => sum + c.capital, 0) + 12000000).toLocaleString()} <span className="text-xs">SDG</span></p>
            ...
            <div className="bg-white p-6 rounded-3xl border border-gray-200 shadow-sm flex flex-col justify-between min-h-[140px] hover:scale-[1.01] transition-all duration-300">
              ...
              <p className="text-3.5xl font-black text-[#1A1A1A] my-2">{applications.length}</p>
            ...
            <div className="bg-white p-6 rounded-3xl border border-gray-200 shadow-sm flex flex-col justify-between min-h-[140px] hover:scale-[1.01] transition-all duration-300">
              ...
              <p className="text-3.5xl font-black text-rose-600 my-2">
                {complaints.length > 0 
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            <div className="bg-white p-6 rounded-3xl border border-gray-200 shadow-sm space-y-4 lg:col-span-2">
              <h4 className="font-extrabold text-[#1A1A1A] text-sm pb-2 border-b border-gray-100">{currentLanguage === "ar" ? "توزيع السجلات التجارية عبر الولايات السودانية" : "Commercial Registries by Sudanese States"}</h4>
              <div className="h-64">
            ...
            <div className="bg-white p-6 rounded-3xl border border-gray-200 shadow-sm space-y-4 flex flex-col justify-between">
              <h4 className="font-extrabold text-[#1A1A1A] text-sm pb-2 border-b border-gray-100">{currentLanguage === "ar" ? "هيكل وتوزيع القطاع الصناعي" : "Industrial Sector Distribution"}</h4>
        ...
        <div className="space-y-6">
          <div className="bg-white p-6 rounded-3xl border border-gray-200 shadow-sm flex items-center justify-between">
            <div className="space-y-1">
              ...
              <h2 className="text-lg font-bold text-[#1A1A1A] flex items-center gap-2 mt-1">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="bg-white p-6 rounded-3xl border border-gray-200 shadow-sm space-y-4">
              <h4 className="font-extrabold text-[#1A1A1A] text-sm pb-2 border-b border-gray-100">{currentLanguage === "ar" ? "أداء التفتيش والتحقق من التراخيص" : "Inspection & License Audits"}</h4>
              <div className="h-56">
                <ResponsiveContainer width="100%" height="100%">
            ...
            <div className="bg-white p-6 rounded-3xl border border-gray-200 shadow-sm space-y-3 flex flex-col justify-between">
              <h4 className="font-extrabold text-[#1A1A1A] text-sm pb-2 border-b border-gray-100">{currentLanguage === "ar" ? "أحدث المنشآت الصناعية المسجلة ومصادر الطاقة" : "Latest Factories & Energy Sources"}</h4>
              <div className="divide-y divide-gray-100 max-h-56 overflow-y-auto pr-1">
                {factories.slice(-4).map(f => (
                  <div key={f.id} className="py-3 flex items-center justify-between text-xs hover:bg-slate-50 transition-colors px-1 rounded-lg">
                    <div>
                      <p className="font-extrabold text-[#1A1A1A]">{f.factoryName}</p>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div className="bg-white border border-gray-200 p-6 rounded-3xl flex items-center justify-between shadow-sm hover:scale-[1.01] transition-all duration-300">
              <div>
                ...
                <p className="text-3xl font-black text-[#1A1A1A] mt-2">{companies.filter(c => c.status === "pending").length}</p>
              </div>
            ...
            <div className="bg-white border border-gray-200 p-6 rounded-3xl flex items-center justify-between shadow-sm hover:scale-[1.01] transition-all duration-300">
              <div>
                ...
                <p className="text-3xl font-black text-[#1A1A1A] mt-2">{factories.filter(f => f.inspectionStatus === "pending").length}</p>
              </div>
      ...
      {(role === UserRole.BUSINESS_INVESTOR || role === UserRole.PUBLIC_CITIZEN) && (
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            ...
            {/* My companies */}
            <div className="bg-white p-6 rounded-3xl border border-gray-200 shadow-sm space-y-4 flex flex-col justify-between">
              <h4 className="font-extrabold text-[#1A1A1A] text-sm flex items-center gap-2 pb-2 border-b border-gray-100">
              ...
              <div className="space-y-2.5">
                {companies.slice(0, 3).map(c => (
                  <div key={c.id} className="bg-[#F4F6F5] p-3.5 rounded-2xl border border-gray-200 flex justify-between items-center text-xs">
                    <div>
                      <p className="font-extrabold text-[#1A1A1A]">{currentLanguage === "ar" ? c.companyNameAr : c.companyNameEn}</p>
              ...
              <div className="space-y-2.5">
                {applications.slice(0, 3).map(app => (
                  <div key={app.id} className="bg-[#F4F6F5] p-3.5 rounded-2xl border border-gray-200 space-y-2 text-xs">
                    <p className="font-extrabold text-[#1A1A1A] line-clamp-1">{app.proposedProject}</p>
